...
const Responsive = ({ children }: { children: React.ReactNode }) => {
    const [screenSize, setScreenSize] = useState(window.innerWidth > maxWidthCutoff ? 'lg' : 'sm');
    const debouncedSetScreenSize = useCallback(debounce((params: string) => {
        setScreenSize(params);
    }, 150), [debounce, setScreenSize]);

    useEffect(() => {
        const handleResize = () => {
            if (window.innerWidth > maxWidthCutoff) debouncedSetScreenSize('lg');
            else if (window.innerWidth <= maxWidthCutoff) debouncedSetScreenSize('sm');
        }
